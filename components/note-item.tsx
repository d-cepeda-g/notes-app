...
export const NoteItem = React.memo(function NoteItem({
  ...
  const NoteContent = (
    <li
      ...
      className={`h-[70px] w-full max-w-full min-w-0 overflow-hidden box-border ${
    ...
    >
      <Link
        ...
        prefetch={true}
        tabIndex={-1}
        className="flex flex-col justify-center gap-0.5 h-full w-full min-w-0 overflow-hidden px-3 py-2 box-border"
      >
        <h2 className="text-sm font-bold truncate min-w-0">
        ...
        >
          <span className="text-black dark:text-white shrink-0 mr-1">
  ...
  if (isMobile) {
    return (
      <div {...handlers} className="relative overflow-hidden w-full max-w-full min-w-0 box-border">
        <div
          className={`transition-transform duration-300 ease-out w-full max-w-full min-w-0 overflow-hidden box-border ${
