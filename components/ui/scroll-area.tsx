...
>(({ className, children, withVerticalMargins = false, mobileHeaderHeight = false, isMobile = false, ...props }, ref) => (
  ...
  >
    <ScrollAreaPrimitive.Viewport 
      className={cn(
        "h-full w-full rounded-[inherit] box-border",
        isMobile && "!w-full !max-w-[100vw] overflow-x-hidden"
      )}
      style={isMobile ? { width: '100%', maxWidth: '100vw', boxSizing: 'border-box' } : undefined}
    >
      {children}
    </ScrollAreaPrimitive.Viewport>
